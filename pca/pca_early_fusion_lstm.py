...
def k_PCA(X, varRetained = 0.95):
    '''Computing the d-dimensional mean vector'''
    mean_vec = np.mean(X, axis=0)
    cov_mat = (X-mean_vec).T.dot((X-mean_vec)) / (X.shape[0]-1)
    eig_vals, eig_vecs = np.linalg.eig(cov_mat)
#assertion issue with decimal places, so commenting this out
#    for ev in eig_vecs:
#        np.testing.assert_array_almost_equal(float(1.0), float(np.linalg.norm(ev)))
    # List of (eigenvalue, eigenvector) tuples
    eig_pairs = [(np.abs(eig_vals[i]), eig_vecs[:,i]) for i in range(len(eig_vals))]
    # Sort the tuples from high to low
    eig_pairs = list(reversed(sorted(eig_pairs)))

...
pig = lambda x: np.asarray([j[1:35] for i,k,j in x])
train_set_audio = np.array([np.mean(pig(covarep['covarep'][vid][sid]),axis=0) for (vid, sid) in train_set_ids if covarep['covarep'][vid][sid]])
valid_set_audio = np.array([np.mean(pig(covarep['covarep'][vid][sid]),axis=0) for (vid, sid) in valid_set_ids if covarep['covarep'][vid][sid]])

train_set_visual = np.array([np.mean(pig(facet['facet'][vid][sid]),axis=0) for (vid, sid) in train_set_ids])
valid_set_visual = np.array([np.mean(pig(facet['facet'][vid][sid]),axis=0) for (vid, sid) in valid_set_ids])

train_set_audio[train_set_audio == -float("inf")] = 0
valid_set_audio[valid_set_audio == -float("inf")] = 0
train_set_visual[train_set_visual == -float("inf")] = 0
valid_set_visual[valid_set_visual == -float("inf")] = 0
